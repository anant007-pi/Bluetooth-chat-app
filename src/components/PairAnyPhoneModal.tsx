...
export const PairAnyPhoneModal: React.FC<PairAnyPhoneModalProps> = ({
  ...
  const generateQr = async (code: string) => {
    try {
      ...
      const pairUrl = `${origin}?pair=${encodeURIComponent(code)}`;
  ...
  const handleCopyLink = () => {
    ...
    const pairUrl = `${origin}?pair=${encodeURIComponent(roomCode)}`;
            ...
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <div className="w-full max-w-sm space-y-2 text-left text-xs text-slate-400">
                <div className="flex items-start gap-2 bg-slate-800/40 p-2.5 rounded-xl border border-slate-800">
                  ...
                  <p>
                    Open the camera app on <strong>iPhone</strong> or <strong>Android</strong> and point it at the QR code.
                  </p>
                </div>
                ...
                <div className="flex items-start gap-2 bg-slate-800/40 p-2.5 rounded-xl border border-slate-800">
                  ...
                  <p>
                    Tap the link to join. Both devices will pair automatically and exchange messages over the wireless Bluetooth relay!
                  </p>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full max-w-sm flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold transition-colors"
              >
                <Share2 className="w-4 h-4 text-blue-400" />
                <span>Share Pairing Link via AirDrop / WhatsApp / SMS</span>
              </button>
            </div>
          )}
